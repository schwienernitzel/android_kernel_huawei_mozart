...
/*mali related io remap addr table*/
mali_soc_remap_addr_table *gp_mali_soc_addr_table = NULL;
static _mali_osk_errcode_t mali_remap_soc_addr();
static void mali_unmap_soc_addr();
...
void mali_domain_powerup_finish(void)
{
    ...
    MALI_DEBUG_PRINT(3, ("mali domain power up start! \n"));

    mali_reg_writel(gp_mali_soc_addr_table->soc_ao_sctrl_base_addr,SOC_AO_SCTRL_SC_PW_RSTDIS0_ADDR(0),1,1,1);
    ret = mali_reg_readl(gp_mali_soc_addr_table->soc_ao_sctrl_base_addr, SOC_AO_SCTRL_SC_PW_RST_STAT0_ADDR(0), 1, 1);
    if(0 != ret)
    {
        MALI_DEBUG_PRINT(2, (" error:  SET SC_PW_RSTDIS0 failed!\n"));
    }

    mali_reg_writel(gp_mali_soc_addr_table->soc_ao_sctrl_base_addr,SOC_AO_SCTRL_SC_PW_ISODIS0_ADDR(0),1,1,1);
    ret = mali_reg_readl(gp_mali_soc_addr_table->soc_ao_sctrl_base_addr, SOC_AO_SCTRL_SC_PW_ISO_STAT0_ADDR(0), 1, 1);
    if(0 != ret)
    {
        MALI_DEBUG_PRINT(2, (" error:  SET SC_PW_ISODIS0 failed!\n"));
    }

    mali_reg_writel(gp_mali_soc_addr_table->soc_ao_sctrl_base_addr,SOC_AO_SCTRL_SC_PW_CLKEN0_ADDR(0),1,1,1);
    ret = mali_reg_readl(gp_mali_soc_addr_table->soc_ao_sctrl_base_addr, SOC_AO_SCTRL_SC_PW_CLK_STAT0_ADDR(0), 1, 1);
    if(1 != ret)
    {
        MALI_DEBUG_PRINT(2, (" error:  SET SC_PW_CLKEN0 failed!\n"));
    }

    mali_reg_writel(gp_mali_soc_addr_table->soc_media_sctrl_base_addr,SOC_MEDIA_SCTRL_SC_MEDIA_RSTDIS_ADDR(0),0,0,1);
    ret = mali_reg_readl(gp_mali_soc_addr_table->soc_media_sctrl_base_addr, SOC_MEDIA_SCTRL_SC_MEDIA_RST_STAT_ADDR(0), 0, 0);
    if(0 != ret)
    {
...
void mali_domain_powerdown_begin(void)
{
    ...
    mali_reg_writel(gp_mali_soc_addr_table->soc_media_sctrl_base_addr,SOC_MEDIA_SCTRL_SC_MEDIA_RSTEN_ADDR(0),0,0,1);

    mali_reg_writel(gp_mali_soc_addr_table->soc_media_sctrl_base_addr,SOC_MEDIA_SCTRL_SC_MEDIA_CLKDIS_ADDR(0),1,1,1);

    mali_reg_writel(gp_mali_soc_addr_table->soc_ao_sctrl_base_addr,SOC_AO_SCTRL_SC_PW_CLKDIS0_ADDR(0),1,1,1);

    mali_reg_writel(gp_mali_soc_addr_table->soc_ao_sctrl_base_addr,SOC_AO_SCTRL_SC_PW_ISOEN0_ADDR(0),1,1,1);

    mali_reg_writel(gp_mali_soc_addr_table->soc_ao_sctrl_base_addr,SOC_AO_SCTRL_SC_PW_RSTEN0_ADDR(0),1,1,1);
...
_mali_osk_errcode_t mali_regulator_enable(void)
{
    ...
    for (i = 0; i < 50; i++)
    {
        ret = mali_reg_readl(gp_mali_soc_addr_table->soc_ao_sctrl_base_addr, SOC_AO_SCTRL_SC_PW_MTCMOS_STAT0_ADDR(0), 1, 1);
        if (1 == ret)
        {
    ...
    if (50 == i)
    {
        MALI_DEBUG_PRINT(2, ("mali error: MTCMOS open failed\n"));
        return _MALI_OSK_ERR_FAULT;
    }
}
...
_mali_osk_errcode_t mali_regulator_disable(void)
{
    ...
    if(0 != regulator_disable(mali_regulator))
    {
        MALI_DEBUG_PRINT(1, ("mali error:mali_regulator disable failed!\n"));
        return _MALI_OSK_ERR_FAULT;
    }
...
static mali_bool init_mali_clock_regulator(struct platform_device *pdev)
{
    ...
    if(_MALI_OSK_ERR_OK != mali_regulator_enable())
    {
        return MALI_FALSE;
    }
    ...
    /*使能媒体外设时钟*/
    ret = mali_reg_readl(gp_mali_soc_addr_table->soc_peri_sctrl_base_addr, SOC_PERI_SCTRL_SC_PERIPH_CLKSTAT12_ADDR(0), 10, 10);
    if(1 != ret)
    {
        mali_reg_writel(gp_mali_soc_addr_table->soc_peri_sctrl_base_addr,SOC_PERI_SCTRL_SC_PERIPH_CLKEN12_ADDR(0),10,10,1);
        ret = mali_reg_readl(gp_mali_soc_addr_table->soc_peri_sctrl_base_addr, SOC_PERI_SCTRL_SC_PERIPH_CLKSTAT12_ADDR(0), 10, 10);
        if(1 != ret)
        {
    ...
    /* CLK on and set rate */
    if(_MALI_OSK_ERR_OK != mali_clock_on())
    {
        return MALI_FALSE;
    }
...
_mali_osk_errcode_t mali_platform_powerdown(void)
{
    ...
    if (g_swGpuPowerState != MALI_FALSE)
    {
        ...
        if(_MALI_OSK_ERR_OK != mali_regulator_disable())
        {
            return _MALI_OSK_ERR_FAULT;
        }
...
static void mali_platform_powerup_no_drv(void)
{
    ...
    mali_reg_writel(gp_mali_soc_addr_table->soc_ao_sctrl_base_addr,SOC_AO_SCTRL_SC_PW_MTCMOS_EN0_ADDR(0),1,1,1);

    mali_reg_writel(gp_mali_soc_addr_table->soc_peri_sctrl_base_addr,SOC_PERI_SCTRL_SC_PERIPH_CLKEN12_ADDR(0),0,31,0x400);

    mali_reg_writel(gp_mali_soc_addr_table->soc_pmctl_base_addr,SOC_PMCTRL_GPUPLLCTRL_ADDR(0),0,31,0x7801);

    mali_reg_writel(gp_mali_soc_addr_table->soc_pmctl_base_addr,SOC_PMCTRL_SYSPLLCTRL_ADDR(0),0,31,0x7801);

    mali_reg_writel(gp_mali_soc_addr_table->soc_pmctl_base_addr,SOC_PMCTRL_MEDPLLCTRL_ADDR(0),0,31,0x7801);

    mali_reg_writel(gp_mali_soc_addr_table->soc_media_sctrl_base_addr,SOC_MEDIA_SCTRL_SC_MEDIA_CLKCFG2_ADDR(0),0,31,0x8100);

    mali_reg_writel(gp_mali_soc_addr_table->soc_media_sctrl_base_addr,SOC_MEDIA_SCTRL_SC_MEDIA_CLKCFG0_ADDR(0),0,31,0x84);//0x84
    ...
    mali_reg_writel(gp_mali_soc_addr_table->soc_media_sctrl_base_addr,SOC_MEDIA_SCTRL_SC_MEDIA_CLKEN_ADDR(0),0,31,0x20002);
...
_mali_osk_errcode_t mali_platform_init(struct platform_device *pdev)
{
    ...
    /* open and get the clk and regulator without mali pmm */
    if(MALI_TRUE != init_mali_clock_regulator(pdev))
    {
        return _MALI_OSK_ERR_FAULT;
    }
...
static _mali_osk_errcode_t mali_remap_soc_addr(void)
{
    ...
    gp_mali_soc_addr_table = (mali_soc_remap_addr_table *)kmalloc(sizeof(mali_soc_remap_addr_table), GFP_KERNEL);
    gp_mali_soc_addr_table->soc_media_sctrl_base_addr = ioremap(SOC_MEDIA_SCTRL_BASE_ADDR, REG_MEDIA_SC_IOSIZE);
    gp_mali_soc_addr_table->soc_ao_sctrl_base_addr = ioremap(SOC_AO_SCTRL_BASE_ADDR, REG_SC_ON_IOSIZE);
    gp_mali_soc_addr_table->soc_peri_sctrl_base_addr = ioremap(SOC_PERI_SCTRL_BASE_ADDR, REG_SC_OFF_IOSIZE);
    gp_mali_soc_addr_table->soc_pmctl_base_addr = ioremap(SOC_PMCTRL_BASE_ADDR, REG_PMCTRL_IOSIZE);

    if ((gp_mali_soc_addr_table->soc_media_sctrl_base_addr == NULL)
        || (gp_mali_soc_addr_table->soc_ao_sctrl_base_addr == NULL)
        || (gp_mali_soc_addr_table->soc_peri_sctrl_base_addr == NULL)
        || (gp_mali_soc_addr_table->soc_pmctl_base_addr == NULL)) {
...
static void mali_unmap_soc_addr(void)
{
    ...
    iounmap((void __iomem *)gp_mali_soc_addr_table->soc_media_sctrl_base_addr);
    iounmap((void __iomem *)gp_mali_soc_addr_table->soc_ao_sctrl_base_addr);
    iounmap((void __iomem *)gp_mali_soc_addr_table->soc_peri_sctrl_base_addr);
    iounmap((void __iomem *)gp_mali_soc_addr_table->soc_pmctl_base_addr);
    kfree((void *)gp_mali_soc_addr_table);
    gp_mali_soc_addr_table = NULL;
}
