...
#define UCOM_NV_HSUPA_ETFCI_TABLE_MAX_NUM           ( 6 )                       /* 最大HSUPA ETFCI 表格个数 */
...
#define UCOM_NV_W_APT_FREQ_COMP_NUM                 ( 16 )                      /* APT频率补偿数 */
#define UCOM_NV_W_APT_TEMP_COMP_NUM                 ( 16 )                      /* APT温度补偿数 */
#define UCOM_NV_W_CHANNEL_NUM                       ( 3 )                       /* NV项每个BAND区分高中低三个信道 */
#define UCOM_NV_W_PA_SWITCH_COMP_SLOT               ( 4 )                       /* Pa切换补偿时隙数 */
#define UCOM_NV_W_APC_ATTEN_NUM                     ( 0x58 )                    /* APC衰减数 */
...
#define UCOM_NV_W_TOTAL_BAND_NUM                ( 6 )                           /* band config支持的W频段 */
...
typedef PHY_UINT16 UCOM_NV_RCM_AGENTC_COMPARE_PHASE_ENUM_UINT16;
...
}UCOM_NV_PRODUCT_TYPE_FOR_LOG_STRU;
...
typedef struct
{
    PHY_INT16                           ashwGsmSarBackoffPower[UCOM_NV_SAR_BACKOFF_NUM][UCOM_NV_SAR_MODEM_SCENE_NUM];
...
typedef struct
{
    PHY_UINT32                          uwTunerMask;                            /* Tuner的掩码，如TUNER使用BIT2,BIt3作为线控，则掩码为BIT2|BIT3 */
    PHY_UINT32                          uwReserved;                             /* 保留位 */
...
} UCOM_NV_WG_ANT_MIPI_USID_CHANGE_STRU;
...
/*****************************************************************************
 结构名    : UCOM_NV_W_RF_BAND_CONFIG_STRU
 协议表格  :
 ASN.1描述 :
 结构说明  : W扩展band config
*****************************************************************************/
typedef struct
{
    UCOM_NV_W_BAND_RF_CONFIG_STRU       astWcdmaRfBandConfig[UCOM_NV_W_RF_BAND_CONFIG_NUM];     /* 包括了10个W频段的配置值 */
...
}UCOM_NV_WG_ANT_SWITCH_CONTROL_MODE_STRU;
...
typedef struct
{
    PHY_UINT16                          uhwTasChoiceData[UCOM_NV_RCM_TAS_TABLE_NUM];
    PHY_UINT32                          uwTasScoreData[UCOM_NV_RCM_TAS_TABLE_NUM];
...
typedef struct
{
    PHY_UINT16                          auhwCompareNum[UCOM_NV_RCM_AGENTC_COMPARE_PHASE_BUTT];      /* 采样个数,测量报告个数 */
    PHY_UINT16                          auhwDeltaAveRxAGC[UCOM_NV_RCM_AGENTC_COMPARE_PHASE_BUTT];   /* 差值门限,单位dB */
    PHY_UINT16                          auhwFrozenNum[UCOM_NV_RCM_AGENTC_COMPARE_PHASE_BUTT];       /* 冻结时间乘数，冻结时间=num*1X测量上报周期 */
    PHY_UINT16                          uhwFinalFrozenNum;                                          /* 比较三个子阶段都不满足异常保护时间乘数 */
    PHY_UINT16                          uhwSPassThreshold;                                          /* SPass次数门限 */
    PHY_UINT16                          uhwSFailThreshold;                                          /* SFail次数门限 */
...
typedef struct
{
    PHY_INT16                                   ashwWcdmaSarBackoffPower[UCOM_NV_SAR_BACKOFF_NUM][UCOM_NV_SAR_MODEM_SCENE_NUM];
...
typedef struct
{
    PHY_UINT32                                  uwSupportBandBitmap;            /* 当特殊场景生效时，优先查看对应的频段是否支持特殊场景，如果支持，则使用本NV项进行功率回退。 */
...
typedef struct
{
    PHY_UINT32                                  uwBandMask;                     /* 对应的bit置位1代表高频段。BIT0表示850M,BIT1表示900M,BIT2表示1800M,BIT3表示1900M, */
...
typedef struct
{
    PHY_UINT32                                  uwBandMask;                     /* 对应的bit置位1代表高频段。BIT0表示B1，BIT1表示B2，BIT2表示B3，以此类推 */
...
typedef struct
{
    PHY_UINT16                              uhwPaOffToAntExchangeTime;          /* 关闭PA到输出DPDT信号时间 */
    PHY_UINT16                              uhwAntExchangeToPaOnTime;           /* 输出DPDT信号到打开PA时间 */
...
typedef struct
{
    ...
    PHY_UINT32                              uwNotchGpioDefaultValue;            /* 线控默认值*/
...
typedef struct
{
    PHY_UINT32                                  uwAntSwitch;                    /* 空闲时隙时天线的开关值 */
    PHY_UINT32                                  uwReserved;                     /* 保留字段 */
    UCOM_NV_WG_MIPI_ANT_CMD_ADDRESS_STRU        stAntIdleMipiAddress;           /* Mipi地址 */
    UCOM_NV_WG_MIPI_ANT_CTRL_WORD_STRU          stAntIdleMipiCtrlWord;          /* Mipi控制字 */
...
typedef struct
{
    PHY_UINT32                              uwProfileId;                        /* 根据使用场景，控制前端器件的上下电（ABB，TCXO，RF）以及RF通道的控制。
                                                                                   由AT命令下发配置。默认值为0。取值范围0-7。 */
    PHY_UINT32                              auwReserved[3];                     /* 保留，将来扩展使用 */
...
/*****************************************************************************
 结构名    : UCOM_NV_WCDMA_ANT2_AUTOSWITCH_CTRL_STRU
 协议表格  :
 ASN.1描述 :
 结构说明  : WCDMA动态分集控制
*****************************************************************************/
...
typedef struct
{
    PHY_UINT16                              uhwABBSwitch;                       /* 用于ABB PLL开关的控制。0:ABB CH0 1:ABB CH1 2:ABB CH0&CH1都打开 */
    PHY_UINT16                              uhwRFSwitch;                        /* 用于RFIC电源开关的控制。0:RFIC使用MIPI0控制供电方式 1：RFIC使用MIPI1控制供电方式 2：同时打开两路电源。*/
    PHY_UINT16                              uhwTCXOSwitch;                      /* 0:TCXO0 1:TCXO1 */
    PHY_UINT16                              uhwReserved;                        /* 保留，将来扩展使用 */
...
typedef struct
{
    UCOM_NV_MODE_BASIC_PARA_STRU           stModeBasicPara[2];                  /* 下标[0]:表示GSM模式下的前端器件的上下电控制。
                                                                                   下标[1]:表示WCDMA模式下的前端器件的上下电控制。
                                                                                    注：副卡时，不使用WCDMA模式的配置。*/
    PHY_UINT32                              uwRfSwitch;                         /* 用于控制共分集的开关 */
    PHY_UINT32                              uwGsmRficSel;                       /* 信令模式下当前使用的通道（0：RF0,1：RF1） */
    PHY_UINT32                              uwGpioCtrl;                         /* 是否需要时钟切换(0:不需要, 1:需要 )*/
    PHY_UINT32                              auwReserved[14];                    /* 保留，将来扩展使用 */
...
typedef struct
{
    UCOM_NV_TRI_MODE_CHAN_PARA_STRU          stPara[UCOM_NV_TRI_MODE_CHAN_PARA_PROFILE_NUM];  /* 最多支持8个场景的配置 */
