...
/*lint  --e{537,451,767,718,746,740,958,440}*/
...
VOS_VOID USIMM_SaveLogFile(VOS_VOID)
{
 ...
 /* coverity[uninit_use] */
    ...
    (VOS_VOID)DRV_FILE_LSEEK(ctrlfp, 0, NV_FILE_SEEK_SET);
...
VOS_VOID USIMM_InitNeedPinSteps(USIMM_MODE_TYPE_ENUM_UINT8 ucMode)
{
    ...
    {
        USIMM_ERROR_LOG("USIMM_InitNeedPinSteps: USIMM_InsertNeedPinFileToPool Error");/* [false alarm]:fortify */
    }
...
VOS_VOID USIMM_RefreshIsimFileIndHandle(PS_USIM_REFRESH_IND_STRU *pstRefreshMsg)
{
    ...
    {
        if (USIMM_UMTS_APP == pstRefreshMsg->astEfId[i].enAppType)/* [false alarm]:fortify */
        {
            /* IMSI对应3个ISIM文件 */
            if (EFIMSI == pstRefreshMsg->astEfId[i].usFileId)/* [false alarm]:fortify */
            {
            ...
            /* 判断是否为ISIM对应文件 */
            ausEfId[usFileNum] = USIMM_UsimEFIDToIsimEFID(pstRefreshMsg->astEfId[i].usFileId);/* [false alarm]:fortify */
