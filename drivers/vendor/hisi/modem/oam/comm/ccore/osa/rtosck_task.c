...
VOS_VOID VOS_TaskCtrlBlkInit(VOS_VOID)
{
    ...
    for(i=0; i<(int)vos_TaskCtrlBlkNumber; i++)
    {
        ...
        vos_TaskCtrlBlk[i].ulRtosckTID     = VOS_TASK_CTRL_BLK_VXWORKS_TASKID_NULL;
...
VOS_UINT32 VOS_SuspendTask( VOS_UINT32 ulTaskID )
{
    ...
    else
    {
        ...
        if(ulTaskPID == VOS_TASK_CTRL_BLK_VXWORKS_TASKID_NULL)
        {
            return(VOS_ERR);
        }
    }
...
VOS_UINT32 VOS_TaskDelay( VOS_UINT32 ulMillSecs )
{
    ...
    ulResult = SRE_TaskDelay((int)ulMillSecs/10 + 1);
