...
VOS_VOID CSIMA_DataHook(VOS_UINT32 ulLength, VOS_UINT8 *pucData)
{
    MsgBlock     *pMsg;
    VOS_UINT32   ulResult;
    ...
    if(VOS_OK != ulResult)
    {
        USIMM_WARNING_LOG("CSIMA_DataHook: vos_MsgHook is Failed");
    }
...
VOS_UINT32  CSIMA_OpenChanCnfProc(PS_USIM_OPEN_CHANNEL_CNF_STRU *pstMsg)
{
    ...
    if (VOS_OK != pstMsg->ulResult)
    {
        CSIMA_CardStatusMsgSend((VOS_UINT16)CSIMA_CBP_CARD_NOT_READY, CSIMA_CBP_UNKNOWN_CARD);
...
VOS_VOID CSIMA_FirstCardStatusInd(PS_USIM_STATUS_IND_STRU *pstMsg)
{
    ...
    {
        ulResult = (VOS_INT32)DRV_USIMMSCI_GET_ATR((VOS_ULONG*)&ulATRLen, aucATRData, &stSCIATRInfo);

        if ((VOS_OK != ulResult)||(ulATRLen > SI_ATR_MAX_LEN))
		{
            /* 上报卡状态不可用 */
