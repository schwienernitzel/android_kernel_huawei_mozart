...
COMM_NV_TRI_MODE_FEM_PROFILE_ID_STRU    g_stChannelModeProfileId;

COMM_NV_TRI_MODE_FEM_CHAN_PROFILE_STRU  g_stChannelModeProfile;
...
VOS_UINT32 SLEEP_MasterWakeSlave(SLEEP_MODE_ENUM_UINT32 enRatMode)
{
    switch (enRatMode)
    {
        ...
        case VOS_RATMODE_GSM:
            SLEEP_IsrProc(SLEEP_WAKE_SLAVE_GU_IND, VOS_RATMODE_WCDMA);
            break;
...
VOS_VOID SLEEP_HookMsgProc(MsgBlock *pstMsg)
{
    ...
    if (DSP_PID_WPHY == pstMsg->ulReceiverPid)
    {
        ...
        {
            SLEEP_IsrProc(SLEEP_WAKE_MASTER_GU_IND, VOS_RATMODE_WCDMA);
        }
    ...
    if (DSP_PID_GPHY == pstMsg->ulReceiverPid)
    {
        ...
        {
            SLEEP_IsrProc(SLEEP_WAKE_MASTER_GU_IND, VOS_RATMODE_GSM);
        }
        else if (SLEEP_SLAVE_LP_STATE == g_astSleepModeInfo[VOS_RATMODE_GSM].enSystemState)
        {
            SLEEP_IsrProc(SLEEP_WAKE_SLAVE_GU_IND, VOS_RATMODE_GSM);
        }
        else
        {
        }
...
VOS_VOID SLEEP_Init(VOS_VOID)
{
    ...
    MODEM_ID_ENUM_UINT16                enModem;
    COMM_NV_TRI_MODE_ENABLE_STRU        stRfSwitchFlag;
    ...
    if(NV_OK != NV_Read(en_NV_Item_TRI_MODE_ENABLE, &stRfSwitchFlag, sizeof(COMM_NV_TRI_MODE_ENABLE_STRU)))
    {
    ...
    /* 读取全网通Profile Index控制项 */
    if(NV_OK != NV_Read(en_NV_Item_TRI_MODE_FEM_PROFILE_ID, &g_stChannelModeProfileId, sizeof(COMM_NV_TRI_MODE_FEM_PROFILE_ID_STRU)))
    {
    ...
    /* 读取全网通场景模式通道配置项 */
    if(NV_OK != NV_Read(en_NV_Item_TRI_MODE_FEM_CHAN_PROFILE, &g_stChannelModeProfile, sizeof(COMM_NV_TRI_MODE_FEM_CHAN_PROFILE_STRU)))
    {
    ...
    VOS_MemCpy(&g_stSleepChanPara, &g_stChannelModeProfile.stPara[g_stChannelModeProfileId.ulProfileId].stModeBasicPara[0], sizeof(NV_GUMODE_CHAN_PARA_STRU));

#if defined (INSTANCE_1)
#else
    if (VOS_TRUE == stRfSwitchFlag.usEnable)
    {
        if (VOS_TRUE == g_stChannelModeProfile.stPara[g_stChannelModeProfileId.ulProfileId].ulGpioCtrl)
        {
            /* 全网通功能需要开机时配置射频管脚 */
    ...
    /* 全网通功能需要开机时配置底软上下电通道 */
    if ( VOS_OK != mdrv_pm_gbbp_select_clksrc((PWC_COMM_MODEM_E)enModem, (PWC_COMM_CHANNEL_E)g_stSleepChanPara.astModeBasicParam[PWC_COMM_MODE_GSM].usABBSwitch))
    {
        VOS_ProtectionReboot(DRX_FULLNET_SET_ERROR, VOS_FILE_ID, __LINE__, VOS_NULL_PTR, VOS_NULL);
