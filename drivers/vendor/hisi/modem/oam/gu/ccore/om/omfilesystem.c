...
{
    {OM_QueryDir,     APP_OM_QUERY_DIR_REQ,      OM_APP_QUERY_DIR_IND},
    {OM_ScanDir,      APP_OM_SCAN_DIR_REQ,       OM_APP_SCAN_DIR_IND},
    {OM_OpenFile,     APP_OM_OPEN_FILE_REQ,      OM_APP_OPEN_FILE_CNF},
    {OM_ReadFile,     APP_OM_READ_FILE_REQ,      OM_APP_READ_FILE_IND},
    {OM_WriteFile,    APP_OM_WRITE_FILE_REQ,     OM_APP_WRITE_FILE_CNF},
    {OM_MakeDir,      APP_OM_MAKE_DIR_REQ,       OM_APP_MAKE_DIR_CNF},
    {OM_DeleteFile,   APP_OM_DELETE_FILE_REQ,    OM_APP_DELETE_FILE_CNF},
    {OM_QuerySpace,   APP_OM_QUERY_SPACE_REQ,    OM_APP_QUERY_SPACE_CNF}
};
...
VOS_UINT32 OM_QueryDir(APP_OM_MSG_EX_STRU *pstAppToOmMsg, VOS_UINT16 usReturnPrimId)
{
    ...
    {
        PS_LOG(WUEPS_PID_OM, 0, PS_PRINT_ERROR, "OM_QueryDir:VOS_MemAlloc fail!");
        OM_FILE_SEND_RESULT(pstAppToOmMsg->ucCpuId, pstAppToOmMsg->ucFuncType, OM_FILE_MEM_ALLOC_ERR, usReturnPrimId);
        return VOS_ERR;
    }
    pstOmQueryDir = (OM_APP_QUERY_DIR_STRU*)(pstOmToAppMsg->aucPara);
    /*判断请求文件夹的类型*/
    ...
    if (OM_ROOT_FOLDER == ulFolderType)
    {
                                               ...
                                               pstOmQueryDir->ulLength))
        {
            ...
            PS_LOG1(WUEPS_PID_OM, 0, PS_PRINT_ERROR,
                    "OM_QueryDir: Get fs root directory from Drv fail!", (VOS_INT32)ulFolderType);
            OM_FILE_SEND_RESULT(pstAppToOmMsg->ucCpuId, pstAppToOmMsg->ucFuncType, OM_DRV_GET_DIRECTORY_ERR, usReturnPrimId);
    ...
    else
    {
        ...
        PS_LOG1(WUEPS_PID_OM, 0, PS_PRINT_WARNING,
                  "OM_QueryDir:Unknown ulFolderType fail!", (VOS_INT32)ulFolderType);
        OM_FILE_SEND_RESULT(pstAppToOmMsg->ucCpuId, pstAppToOmMsg->ucFuncType, OM_FILE_RECEIVE_FOLDER_TYPE_ERR, usReturnPrimId);
                                ...
                                OM_APP_SCAN_DIR_STRU *pstOmScanDir)
{
    ...
    VOS_MemSet(&stOmItemInfo, 0, sizeof(stOmItemInfo));
...
VOS_UINT32 OM_ScanDir(APP_OM_MSG_EX_STRU *pstAppToOmMsg, VOS_UINT16 usReturnPrimId)
{
    ...
    {
        PS_LOG(WUEPS_PID_OM, 0, PS_PRINT_ERROR, "OM_ScanDir:opendir fail!");
        OM_FILE_SEND_RESULT(pstAppToOmMsg->ucCpuId, pstAppToOmMsg->ucFuncType, OM_DRV_FILE_OPENDIR_ERR, usReturnPrimId);
        return VOS_ERR;
    }
    /*得到目录内，文件和文件夹的总个数、总的名字长度*/
    OM_GetDirInfo(pstTmpDir, &ulTotalNameLen, &ulTotalNum);
    ...
    {
        PS_LOG(WUEPS_PID_OM, 0, PS_PRINT_ERROR, "OM_ScanDir:closedir fail!");
        OM_FILE_SEND_RESULT(pstAppToOmMsg->ucCpuId, pstAppToOmMsg->ucFuncType, OM_DRV_FILE_CLOSEDIR_ERR, usReturnPrimId);
        return VOS_ERR;
    }
    /*计算返回给工具侧消息包的长度*/
    ulTotalSize = OM_APP_MSG_EX_LEN + OM_APP_SCAN_DIR_HEAD_LEN
    ...
    if (VOS_OK != ulRest)
    {
        VOS_MemFree(WUEPS_PID_OM, pstOmToAppMsg);
        DRV_FILE_CLOSEDIR(pstTmpDir);
        OM_FILE_SEND_RESULT(pstAppToOmMsg->ucCpuId, pstAppToOmMsg->ucFuncType, ulRest, usReturnPrimId);
        return VOS_ERR;
    }
    DRV_FILE_CLOSEDIR(pstTmpDir);
    pstOmToAppMsg->usLength = (VOS_UINT16)(ulTotalSize - VOS_OM_HEADER_LEN);
...
VOS_UINT32 OM_OpenFile(APP_OM_MSG_EX_STRU *pstAppToOmMsg, VOS_UINT16 usReturnPrimId)
{
    ...
    if (DRV_FILE_NULL != g_stOmFileInfo.lFile)
    {
        PS_LOG(WUEPS_PID_OM, 0, PS_PRINT_WARNING,
                               "OM_OpenFile:Last Operation hasn't finished.");
        OM_FILE_SEND_RESULT(pstAppToOmMsg->ucCpuId, pstAppToOmMsg->ucFuncType, OM_DRV_FILE_OPERATE_NOT_FINISH, usReturnPrimId);
        return VOS_ERR;
    }
...
/*lint -e553*/
    ...
    if (VOS_TRUE == NV_CheckNVFile(pcPath))
    {
        ...
        if (LEVEL_ADVANCED != OM_QueryPrivilegeLevel())
        {
            PS_LOG(WUEPS_PID_OM, 0, PS_PRINT_WARNING,
                               "OM_OpenFile:IMEI verify fail.");
            OM_FILE_SEND_RESULT(pstAppToOmMsg->ucCpuId, pstAppToOmMsg->ucFuncType, OM_FILE_OPEN_SC_ERR, usReturnPrimId);
            return VOS_ERR;
        }
    }
#endif
/*lint +e553*/
    ...
    if (VOS_YES == DRV_SEC_CHECK())
    {
        ...
        {
            PS_LOG(WUEPS_PID_OM, 0, PS_PRINT_WARNING, "OM_OpenFile:CFD NV File is secured.");
            OM_FILE_SEND_RESULT(pstAppToOmMsg->ucCpuId, pstAppToOmMsg->ucFuncType, OM_FILE_OPERATE_SEC_FILE_FORBID_CFD, usReturnPrimId);
            return VOS_ERR;
        }
        ...
        {
            PS_LOG(WUEPS_PID_OM, 0, PS_PRINT_WARNING, "OM_OpenFile:ZSP File is secured.");
            OM_FILE_SEND_RESULT(pstAppToOmMsg->ucCpuId, pstAppToOmMsg->ucFuncType, OM_FILE_OPERATE_SEC_FILE_FORBID_ZSP, usReturnPrimId);
            return VOS_ERR;
        }
    }
    ...
    if (DRV_FILE_NULL == g_stOmFileInfo.lFile)
    {
        OM_CloseFile();
        PS_LOG(WUEPS_PID_OM, 0, PS_PRINT_WARNING, "OM_OpenFile:open file fail.");
        OM_FILE_SEND_RESULT(pstAppToOmMsg->ucCpuId, pstAppToOmMsg->ucFuncType, OM_DRV_FILE_OPEN_ERR, usReturnPrimId);
    ...
    if (VOS_ERR == OM_START_FILETIMER(&g_stOmFileInfo.hTimer))
    {
        OM_CloseFile();
        PS_LOG(WUEPS_PID_OM, 0, PS_PRINT_WARNING, "OM_OpenFile:Start Timer failed.");
        OM_FILE_SEND_RESULT(pstAppToOmMsg->ucCpuId, pstAppToOmMsg->ucFuncType, OM_FILE_START_FILE_TIMER_ERR, usReturnPrimId);
        return VOS_ERR;
    }

    OM_SendResultChannel((OM_LOGIC_CHANNEL_ENUM_UINT32)pstAppToOmMsg->ucCpuId, pstAppToOmMsg->ucFuncType, VOS_OK, usReturnPrimId);
    return VOS_OK;
}
/*****************************************************************************
函 数 名  : OM_ReadFile
...
VOS_UINT32 OM_ReadFile(APP_OM_MSG_EX_STRU *pstAppToOmMsg, VOS_UINT16 usReturnPrimId)
{
    ...
    if (DRV_ERROR == (VOS_INT32)ulReadSize)
    {
        ...
        OM_CloseFile();
        PS_LOG(WUEPS_PID_OM, 0, PS_PRINT_ERROR, "OM_ReadFile:read fail!");
        OM_FILE_SEND_RESULT(pstAppToOmMsg->ucCpuId, pstAppToOmMsg->ucFuncType, OM_DRV_FILE_READ_ERR, usReturnPrimId);
        return VOS_ERR;
    }
...
VOS_UINT32 OM_WriteFile(APP_OM_MSG_EX_STRU *pstAppToOmMsg, VOS_UINT16 usReturnPrimId)
{
    ...
    if (DRV_ERROR == lResult)
    {
        OM_CloseFile();
        PS_LOG(WUEPS_PID_OM, 0, PS_PRINT_ERROR, "OM_WriteFile:DRV_FILE_WRITE fail!");
        OM_FILE_SEND_RESULT(pstAppToOmMsg->ucCpuId, pstAppToOmMsg->ucFuncType, OM_DRV_FILE_WRITE_ERR, usReturnPrimId);
        return VOS_ERR;
    }
    /*写入长度不正确*/
    if (lResult != (VOS_INT32)ulWriteSize)
    {
        OM_CloseFile();
        PS_LOG(WUEPS_PID_OM, 0, PS_PRINT_ERROR,
               "OM_WriteFile: the written size is not correct!");
        OM_FILE_SEND_RESULT(pstAppToOmMsg->ucCpuId, pstAppToOmMsg->ucFuncType, OM_DRV_FILE_WRITE_LEN_ERR, usReturnPrimId);
        return VOS_ERR;
    }
    /*启动定时器*/
    if (VOS_OK != OM_START_FILETIMER(&g_stOmFileInfo.hTimer))
...
VOS_UINT32 OM_MakeDir(APP_OM_MSG_EX_STRU *pstAppToOmMsg, VOS_UINT16 usReturnPrimId)
{
    ...
    {
        PS_LOG(WUEPS_PID_OM, 0, PS_PRINT_ERROR, "OM_MakeDir: DRV_FILE_MKDIR failed!");
        OM_FILE_SEND_RESULT(pstAppToOmMsg->ucCpuId, pstAppToOmMsg->ucFuncType, OM_DRV_FILE_MKDIR_ERR, usReturnPrimId);
...
VOS_UINT32 OM_DeleteFile(APP_OM_MSG_EX_STRU *pstAppToOmMsg, VOS_UINT16 usReturnPrimId)
{
    ...
    if (OM_ITEM_FOLDER == ulItemType)
    {
        ...
        {
            PS_LOG(WUEPS_PID_OM, 0, PS_PRINT_ERROR, "OM_DeleteFile: DRV_FILE_RMDIR failed!");
            OM_FILE_SEND_RESULT(pstAppToOmMsg->ucCpuId, pstAppToOmMsg->ucFuncType, OM_DRV_FILE_RMDIR_ERR, usReturnPrimId);
            return VOS_ERR;
        }
        OM_SendResultChannel((OM_LOGIC_CHANNEL_ENUM_UINT32)pstAppToOmMsg->ucCpuId, pstAppToOmMsg->ucFuncType, VOS_OK, usReturnPrimId);
        return VOS_OK;
    ...
    else if (OM_ITEM_FILE == ulItemType)
    {
        ...
        {
            PS_LOG(WUEPS_PID_OM, 0, PS_PRINT_ERROR, "OM_DeleteFile: DRV_FILE_RMFILE failed!");
            OM_FILE_SEND_RESULT(pstAppToOmMsg->ucCpuId, pstAppToOmMsg->ucFuncType, OM_DRV_FILE_RMFILE_ERR, usReturnPrimId);
            return VOS_ERR;
        }
        OM_SendResultChannel((OM_LOGIC_CHANNEL_ENUM_UINT32)pstAppToOmMsg->ucCpuId, pstAppToOmMsg->ucFuncType, VOS_OK, usReturnPrimId);
        return VOS_OK;
    ...
    {
        PS_LOG(WUEPS_PID_OM, 0, PS_PRINT_ERROR, "OM_DeleteFile: Unknown type!");
        OM_FILE_SEND_RESULT(pstAppToOmMsg->ucCpuId, pstAppToOmMsg->ucFuncType, OM_FILE_RECEIVE_ITEM_TYPE_ERR, usReturnPrimId);
    }
    return VOS_ERR;
}
/*****************************************************************************
...
VOS_UINT32 OM_QuerySpace(APP_OM_MSG_EX_STRU *pstAppToOmMsg, VOS_UINT16 usReturnPrimId)
{
    ...
    {
        PS_LOG(WUEPS_PID_OM, 0, PS_PRINT_ERROR, "OM_QuerySpace: DRV_FILE_GET_DISKSPACE");
        OM_FILE_SEND_RESULT(pstAppToOmMsg->ucCpuId, pstAppToOmMsg->ucFuncType, OM_DRV_FILE_GET_DISKSPACE_ERR, usReturnPrimId);
    ...
    OM_SendContentChannel((OM_LOGIC_CHANNEL_ENUM_UINT32)pstAppToOmMsg->ucCpuId, pstAppToOmMsg->ucFuncType, (OM_APP_MSG_EX_STRU*)&stOmDiskSpace, usReturnPrimId);
    return VOS_OK;
}
...
VOS_VOID OM_FileSystemMsgProc(OM_REQ_PACKET_STRU *pRspPacket, OM_RSP_FUNC *pRspFuncPtr)
{
    ...
    if (ulIndex == ulTotalNum)
    {
        PS_LOG1(WUEPS_PID_OM, 0, PS_PRINT_INFO,
          "OM_FileSystemMsgProc: PrimId can't be handled", (VOS_INT32)usPrimId);
        OM_FILE_SEND_RESULT(pstAppToOmMsg->ucCpuId, pstAppToOmMsg->ucFuncType, OM_FILE_RECEIVE_REQ_FIND_FUNC_ERR, usPrimId);
    }
    return;
}
