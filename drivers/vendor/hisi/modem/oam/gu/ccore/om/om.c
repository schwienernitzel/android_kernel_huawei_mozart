...
{
    {OM_ConfigMsgProc,        OM_TRACE_FUNC,    LEVEL_NORMAL},
    {OM_TransMsgProc,         OM_AIR_FUNC,      LEVEL_NORMAL},
    {OM_FileSystemMsgProc,    OM_FS_FUNC,       LEVEL_NORMAL},
    {OM_TransMsgProc,         OM_TRANS_FUNC,    LEVEL_NORMAL},
    {OM_ConfigMsgProc,        OM_EVENT_FUNC,    LEVEL_NORMAL},
    {Log_OmMsgProc,           OM_LOG_FUNC,      LEVEL_NORMAL},
    {OM_RfMsgProc,            OM_WRF_FUNC,      LEVEL_NORMAL},
    {OM_RfMsgProc,            OM_GRF_FUNC,      LEVEL_NORMAL},
    {OM_QueryMsgProc,         OM_AUTH_NV_FUNC,  LEVEL_NORMAL},
    {OM_QueryMsgProc,         OM_QUERY_FUNC,    LEVEL_NORMAL},
    {OM_QueryMsgProc,         OM_NV_FUNC,       LEVEL_NORMAL},
    {OM_TransMsgProc,         OM_SIGCAL_FUNC,   LEVEL_NORMAL},
    {VOS_NULL_PTR,            OM_GREEN_FUNC,    LEVEL_NORMAL},
    {VOS_NULL_PTR,            OM_PRINTF_FUNC,   LEVEL_NORMAL},
    {VOS_NULL_PTR,            OM_READSD_FUNC,   LEVEL_NORMAL},
    {OM_TransferMsgProc,      OM_NOSIG_FUNC,    LEVEL_NORMAL},
    {Om_CcpuBbpConfigMsgProc, OM_BBP_FUNC,      LEVEL_NORMAL}
};
...
VOS_UINT32 OM_SendDataMNTNChannel(OMRL_MNTN_TX_CHAN_CTRL_INFO_STRU *pstCtrlInfo, OM_RSP_PACKET_STRU *pucMsg, VOS_UINT16 usMsgLen)
{
    OM_APP_MSG_EX_STRU                  *pstAppMsg;
    ...
    pstCtrlInfo->stUeToPcSucRecord.stTotalData.ulDataLen += usMsgLen;
    pstCtrlInfo->stUeToPcSucRecord.stTotalData.ulNum++;

    if( VOS_FALSE != VOS_CheckInterrupt() )
    {
        pstCtrlInfo->stUeToPcErrRecord.usSendInIsrErr++;
        return VOS_ERR;
    }
    ...
    if ((OM_STATE_ACTIVE == g_ulOMSwitchOnOff)
        || ((OM_QUERY_FUNC == pstAppMsg->ucFuncType)
            &&(OM_APP_ESTABLISH_CNF == pstAppMsg->usPrimId)))
    {
        if(VOS_OK != OMRL_MsgSplit(pstCtrlInfo,(VOS_UINT8*)pucMsg, usMsgLen))
        {
            pstCtrlInfo->stUeToPcErrRecord.stLostData.ulDataLen += usMsgLen;
            pstCtrlInfo->stUeToPcErrRecord.stLostData.ulNum++;
        }
    ...
    pstCtrlInfo->stUeToPcErrRecord.stLinkErrData.ulDataLen += usMsgLen;
    pstCtrlInfo->stUeToPcErrRecord.stLinkErrData.ulNum++;

    return VOS_ERR;
}


VOS_UINT32 OM_SendDataCbtChannel(OM_RSP_PACKET_STRU *pucMsg, VOS_UINT16 usMsgLen)
{
    OM_CBT_MSG_STRU                     *pstCbtMsg;

    /* 链路未建立 */
    if (OM_STATE_IDLE == g_ulCbtSwitchOnOff)
    {
        /* 记录可维可测信息 */
        return VOS_ERR;
    }
    ...
    /* 将消息发到A核OM处理 */
    pstCbtMsg = (OM_CBT_MSG_STRU*)VOS_AllocMsg(WUEPS_PID_OM,
        (((sizeof(OM_CBT_MSG_STRU) + usMsgLen) - VOS_MSG_HEAD_LENGTH) - 4));

    if (VOS_NULL_PTR == pstCbtMsg)
    {
        PS_LOG(WUEPS_PID_OM, 0, PS_PRINT_ERROR, "OM_SendDataCbtChannel: VOS_AllocMsg failed!");

        return VOS_ERR;
    }

    pstCbtMsg->ulReceiverPid      = ACPU_PID_OMAGENT;
    pstCbtMsg->usPrimId           = OM_CBT_SEND_DATA_REQ;
    pstCbtMsg->usLen              = usMsgLen;

    VOS_MemCpy(pstCbtMsg->aucData,  pucMsg, usMsgLen);

    return VOS_SendMsg(WUEPS_PID_OM, pstCbtMsg);
}
VOS_UINT32 OM_SendDataChannel(OM_LOGIC_CHANNEL_ENUM_UINT32 enChannel, OM_RSP_PACKET_STRU *pucMsg, VOS_UINT16 usMsgLen)
{
    OM_APP_MSG_EX_STRU                 *pstAppMsg;
    ...
    if ( (0 == usMsgLen) || (VOS_NULL_PTR == pucMsg ) )
    {
        LogPrint("OM_SendDataChannel: The input parameter is wrong.\n");
    ...
    {
        /* 有可能是校准消息，需要在校准通道广播，因此此处不返回 */
        ulRet = OM_SendDataMNTNChannel(&g_stTxIndCtrlInfo, pucMsg, usMsgLen);
    }

    if (OM_LOGIC_CHANNEL_CNF == enChannel)
    {
        return OM_SendDataMNTNChannel(&g_stTxCnfCtrlInfo, pucMsg, usMsgLen);
    }

    if (OM_LOGIC_CHANNEL_CBT == enChannel)
    {
        return OM_SendDataCbtChannel(pucMsg, usMsgLen);
    }

    pstAppMsg = (OM_APP_MSG_EX_STRU*)pucMsg;
    ucFuncType= (pstAppMsg->ucFuncType) & OM_FUNCID_VALUE_BITS;

    if ((OM_WRF_FUNC == ucFuncType) || (OM_GRF_FUNC == ucFuncType))
    {
        return OM_SendDataCbtChannel(pucMsg, usMsgLen);
    }
...
VOS_VOID OM_SendResultChannel(OM_LOGIC_CHANNEL_ENUM_UINT32 enChannel,VOS_UINT8 ucFuncType, VOS_UINT32 ulResult, VOS_UINT16 usReturnPrimId)
{
    OM_APP_MSG_EX_STRU stOmToAppMsg;
    stOmToAppMsg.ucFuncType = ucFuncType;
    stOmToAppMsg.usLength   = OM_APP_MSG_EX_LEN;
    ...
    stOmToAppMsg.usToolId   = g_usToolId;
    *((VOS_UINT32*)(stOmToAppMsg.aucPara)) = ulResult;
    OM_SendDataChannel(enChannel, (OM_RSP_PACKET_STRU*)&stOmToAppMsg, (VOS_UINT16)sizeof(OM_APP_MSG_EX_STRU));
    return;
}
...
VOS_UINT32 OM_SendData(OM_RSP_PACKET_STRU *pucMsg, VOS_UINT16 usMsgLen)
{
    return OM_SendDataChannel((OM_LOGIC_CHANNEL_ENUM_UINT32)OM_LOGIC_CHANNEL_IND, pucMsg, usMsgLen);
}
...
VOS_VOID OM_SendResult(VOS_UINT8 ucFuncType,
                            VOS_UINT32 ulResult, VOS_UINT16 usReturnPrimId)
{
    OM_SendResultChannel((OM_LOGIC_CHANNEL_ENUM_UINT32)OM_LOGIC_CHANNEL_IND, ucFuncType, ulResult, usReturnPrimId);

    return;
}


VOS_VOID OM_SendContentChannel(OM_LOGIC_CHANNEL_ENUM_UINT32 enChannel, VOS_UINT8 ucFuncType, OM_APP_MSG_EX_STRU *pstOmToAppMsg, VOS_UINT16 usReturnPrimId)
{
    pstOmToAppMsg->ucFuncType = ucFuncType;
    pstOmToAppMsg->usPrimId   = usReturnPrimId;
    pstOmToAppMsg->usToolId   = g_usToolId;
    OM_SendDataChannel(enChannel, (OM_RSP_PACKET_STRU*)pstOmToAppMsg, (VOS_UINT16)(pstOmToAppMsg->usLength + VOS_OM_HEADER_LEN));
    return;
}
...
VOS_VOID OM_SendContent(VOS_UINT8 ucFuncType,
                             OM_APP_MSG_EX_STRU *pstOmToAppMsg, VOS_UINT16 usReturnPrimId)
{
    OM_SendContentChannel((OM_LOGIC_CHANNEL_ENUM_UINT32)OM_LOGIC_CHANNEL_IND, ucFuncType, pstOmToAppMsg, usReturnPrimId);
...
/*****************************************************************************
 ...
 修改历史      :
  1.日    期   : 2009年6月22日
    ...
    修改内容   : 新生成函数
*****************************************************************************/
VOS_UINT32 OM_GreenChannelChannel(OM_LOGIC_CHANNEL_ENUM_UINT32 enChannel, VOS_UINT8 ucFuncType, VOS_UINT16 usPrimId,
                            VOS_UINT8 *pucData, VOS_UINT16 usLen)
{
    OM_APP_TRACE_STRU                   *pstAppMsg;
    VOS_UINT32                          ulResult;
    ...
    ulResult = OM_SendDataChannel(enChannel, (OM_RSP_PACKET_STRU*)pstAppMsg, usLen + OM_APP_TRACE_LEN);
    VOS_MemFree(WUEPS_PID_OM, pstAppMsg);
...
VOS_UINT32 OM_GreenChannel(VOS_UINT8 ucFuncType, VOS_UINT16 usPrimId,
                            VOS_UINT8 *pucData, VOS_UINT16 usLen)
{
    return OM_GreenChannelChannel(OM_LOGIC_CHANNEL_IND, ucFuncType, usPrimId, pucData, usLen);
}
