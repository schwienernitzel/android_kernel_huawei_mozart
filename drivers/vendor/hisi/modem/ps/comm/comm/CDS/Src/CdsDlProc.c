...
#if (CDS_FEATURE_ON == CDS_FEATURE_BASTET)
    /*BASTET*/
    if (PS_TRUE == CDS_IsBastetPacket( pstDlData->pstSdu ))
    {
        /*下行流量统计*/
        ...
        CDS_DlProcBastetData( pstDlData->pstSdu, ucDeftRabId, enModemId, pstDlData->ulSduLen );
        return;
    }
    CDS_BstSendTxRxInd();
#endif
...
#if (CDS_FEATURE_ON == CDS_FEATURE_BASTET)
    /*BASTET*/
    if (PS_TRUE == CDS_IsBastetPacket( pstDsLpdcpData->pstSdu ))
    {
        /*下行流量统计*/
