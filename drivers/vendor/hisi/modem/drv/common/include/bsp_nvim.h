...
struct nv_ctrl_file_info_stru
{
    ...
    u8   crc_mark;        /*crc mark 0: no crc 1 :crc*/
...
{
    u32 count;
    struct nv_flush_info_stru list[NV_FLUSH_LIST_SIZE];
};
struct nv_crc_flush_info_stru
{
    u32 count;
    u32 offsetArray[2*NV_FLUSH_LIST_SIZE];/*存储4K块数据的偏移的数组*/
};
...
struct nv_global_ddr_info_stru
{
    ...
    u32 p_crc_code;                            /*CRC的位置指针，物理地址，使用时需要转换*/
...
/*DDR中CRC校验码的位置指针*/
#define NV_DDR_CRC_CODE_OFFSET    ((u32 *)SHD_DDR_P2V(((struct nv_global_ddr_info_stru*)NV_GLOBAL_INFO_ADDR)->p_crc_code))
/*是否进行CRC校验标志*/
#define NV_CRC_CHECK_YES          (((struct nv_ctrl_file_info_stru*)NV_GLOBAL_CTRL_INFO_ADDR)->crc_mark)
